...
import {
  ...
  StyleSheet, Alert, SafeAreaView, Switch, ActivityIndicator,
} from 'react-native';
...
type RoutePropType = RouteProp<TeacherStackParamList, 'EditLesson'>;
...
export default function EditLessonScreen() {
  ...
  const { isLoading } = useQuery({
    queryKey: ['lesson-detail', params.classId, params.lessonId],
    queryFn: () => classesService.getLesson(params.classId, params.lessonId),
    enabled: !loaded,
  ...
  const { mutate: updateLesson, isPending } = useMutation({
    mutationFn: () => {
      let deadline: string | undefined | null;
      ...
      return classesService.updateLesson(params.classId, params.lessonId, {
        title: title.trim(),
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['lesson-detail', params.classId, params.lessonId] });
      navigation.goBack();
    },
    onError: () => Alert.alert('Error', 'Failed to update lesson. Please try again.'),
  });
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <LinearGradient colors={['#7C3AED', '#5B21B6']} style={styles.header}>
          ...
          <Text style={styles.headerSub}>Update lesson content and settings</Text>
        </LinearGradient>
